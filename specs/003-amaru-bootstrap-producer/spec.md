...
An operator already runs a cardano-node — either a long-syncing mainnet node, a relay on preprod, or a producer node in the antithesis testnet's compose stack. They want to add Amaru to the same machine without performing any pre-snapshot ceremony of their own. They drop the bootstrap-producer container into their compose alongside their existing node, point its read-only mount at the node's chain DB, and `docker compose up`. The bootstrap step opens the chain DB, reads its immutable tip, decides whether the chain is mature enough for amaru to consume (i.e. the tip is in the era amaru implements *and* the snapshot point has at least two preceding epochs of that era available for nonce computation), and:
...
- **FR-001**: The system MUST provide a one-shot bootstrap step that, given read-only access to a cardano-node's chain database (live, write-in-progress, or a finished snapshot — the bootstrap step does not care which), produces the complete Amaru bootstrap bundle on a designated shared volume.
...
- **FR-003**: When the bootstrap step succeeds, every required artefact for the Amaru bundle MUST be on the shared volume and complete; when it fails, NO partial Amaru-readable artefact MUST be left behind that could mislead a downstream consumer.
...
- **Snapshot point (derived)**: the slot at which the bootstrap step produces its snapshot. Defaults to the immutable tip slot at the moment the era-readiness predicate first holds. By construction this is past the chain's volatility horizon (k-blocks back from the chain's "real" tip) — no additional safety margin needed.
- **Amaru bootstrap bundle (output)**: the complete set of artefacts an Amaru node needs to start, derived from the cardano-node's chain state at the snapshot point. Includes a populated chain store, a populated ledger store, a nonces document, and a small set of header documents.
...
- The cardano-node's chain DB volume is mountable read-only into the bootstrap step's container. No special filesystem coordination (locking, snapshotting) is required between the writer (the node) and the reader (the bootstrap step), beyond the orchestrator's standard volume mounting.
