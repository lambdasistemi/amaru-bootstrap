...
**Tests**: Tests are IN scope (TDD per the speckit RPI overlay; bats unit tests for failure paths + hspec round-trip + bats integration via the existing Phase 0 smoke test).
...
- [ ] T001 Add `executable snapshot-emitter` stanza to [`amaru-bootstrap.cabal`](../../../amaru-bootstrap.cabal): `main-is: Main.hs`, `hs-source-dirs: app/snapshot-emitter`, `other-modules: SnapshotEmitter`, `build-depends: base, bytestring, cardano-binary, directory, filepath, ouroboros-consensus, ouroboros-consensus-cardano, amaru-bootstrap`. Hackage-ready per constitution Code Quality Gates (cabal check clean, `-Werror`)
- [ ] T002 [P] Create [`nix/snapshot-emitter.nix`](../../../nix/snapshot-emitter.nix): `project.hsPkgs.amaru-bootstrap.components.exes.snapshot-emitter`. References [research.md R-001](./research.md#r-001-legacy-snapshot-encoder-lives-in-upstreams-exposed-api)
- [ ] T003 Wire `snapshotEmitterPkg` through [`flake.nix`](../../../flake.nix) into the perSystem outputs (alongside existing `amaruPkg`, `iogTools`). Depends on T002
- [ ] T004 Update [`nix/checks.nix`](../../../nix/checks.nix) to expose `snapshot-emitter` as a flake check (just the exe build), and update [`nix/apps.nix`](../../../nix/apps.nix) to add `nix run .#snapshot-emitter` mapping per [contracts/snapshot-emitter-cli.md "Invocation"](./contracts/snapshot-emitter-cli.md#invocation). Depends on T003
